...
**Question: **Without retraining the model, but aiming to reduce inference time by 50% even with a slight compromise in accuracy, which model optimization technique should you initially attempt?
Answer: Dynamic range quantization

**Reason: **Dynamic range quantization is a post-training optimization technique that reduces model size and improves inference speed without retraining. It converts 32-bit floating-point weights to 8-bit integers, while activations are dynamically quantized during inference.
