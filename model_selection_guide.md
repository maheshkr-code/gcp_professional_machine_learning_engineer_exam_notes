### Self reference - Create an internally or externally trained model - Notes /Dec/21/2025


| S.No | Category | Model Name  |  When to use | why  | Sample use case |
| --- | --- | --- | --- | --- | --- |
| 1. | Time series models | (a)ARIMA_PLUS </br> (b)ARIMA_PLUS_XREG  |  You have a single metric tracked over time. | Automatically handles seasonality, holidays, and outliers in a single query | Forecasting next month's daily electricity demand.
| 2. | `Diagnostics`  | `Contribution Analysis`   | You need to explain the "root cause" of a change in data.   | Sifts through millions of segments to find the exact drivers of a metric shift. | Explaining why revenue dropped 12% in the UK specifically among mobile users.
| 3. | Classification  | Logistic Regression   | You need a fast, simple baseline for binary or multi-class labels.   | Highly interpretable; uses a sigmoid function to map data to probabilities (0 to 1). |  Determining if an email is "Spam" or "Not Spam."
| 4. | Boosted Trees (XGBoost) Classification | Classification   | You want to find hidden patterns of groups in your data without prefined labels | Segmenting your customer base into "High Spenders" vs "Bargain Hunters"
| 5. | Random Foresst classification  | Classification   |  | 
| 6. | DNN classification  | Classification   |  | 
| 7. | Wide-and-deep classification  | Classification   |  | 
| 8. | AutoML classification  | Classification   |  | 
| 9. | Linear Regression  | Regression   | You need fast, sinple baseline for numerical predictions   | Predicting the expected price of a home
| 10. | Boosted Linear Regression  | Regression   |    | 
| 11. | Random Foresst classification  | Classification   |  | 
| 12. | DNN classification  | Classification   |  | 
| 13. | Wide-and-deep classification  | Classification   |  | 
| 14. | AutoML classification  | Classification   |  |
| 15. | Clustering | K-means   |  |
| 16. | Recommendation | Matrix factorization   |  |
| 16. | Principal component analysis (PCA)| Dimensionality reduction | |
| 17. | Autoencoder | Dimensionality reduction |  |
